...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  return <>{children}<dialog open className="auth-overlay" aria-labelledby="auth-title"><section className="auth-modal"><span className="eyebrow">安全登录</span><h1 id="auth-title">先验证你的邮箱。</h1><p>我们会发送一封登录链接，用于保护你的测评会话与邀请链接。</p><form className="start-form" onSubmit={(event) => void submit(event)}><label htmlFor="auth-email"><span>邮箱</span></label><Input id="auth-email" type="email" value={email} required onChange={(event) => setEmail(event.target.value)} /><Button className="primary-button" type="submit" disabled={busy}>{busy ? '正在发送…' : '发送登录链接'} <ArrowRight /></Button>{sent && <p>邮件已发送。请在同一设备打开邮件中的链接后返回这里。</p>}{error && <p className="inline-error" role="alert">{error}</p>}</form></section></dialog></>;
}
